...
//      'predictionOffset': 'prediction offset',
        ...
        class Context {
                ...
                if (actions['redraw']) {
                    stages['redraw'] = stages['update.facets']
                        ...
                        .then(() => {
                            ...
                            c.viewTable.on('PanZoom', (ev) => ActivityLogger.log({
                                'context': c.getNameAndUUID(),
                                'changedAxis': ev
                            }, 'PanZoom'));
                        });
            ...
            static _makeShelvesGUI(context) {
                ...
                for (const key of Object.keys(shelves)) {
                    ...
                    shelves[key].on(sh.Shelf.Event.Add, record => ActivityLogger.log({
                        shelf: record.shelf.type,
                        what: record.content.toJSON(),
                        'context': context.getNameAndUUID()
                    }, sh.Shelf.Event.Add));
                    shelves[key].on(sh.Shelf.Event.Remove, record => ActivityLogger.log({
                        shelf: record.shelf.type,
                        what: record.content.toJSON(),
                        'context': context.getNameAndUUID()
                    }, sh.Shelf.Event.Remove));
                }
                return visual;
            }
                            ...
                            (what, idx) => {
                                ...
                                let $checkBox = $('<input type="checkbox">')
                                    ...
                                    .change((e) => {
                                        ...
                                        // log user activity
                                        ActivityLogger.log({
                                            'changedFacet': _facetNameMap[what],
                                            'value': e.target.checked,
                                            'facets': context._getFacetActiveState(),
                                            'context': context.getNameAndUUID()
                                        }, "facet.change");
                                        // ... trigger an update
        ...
        class Toolbar {
            ...
            static _makeToolbarButton(iconName, label) {
                    ...
                    if (widget === undefined && hasContext)
                        ...
                        return context.model.pciGraph_get().then(
                            graph => {
                                ...
                                widget.on('Node.DragMoved', node => ActivityLogger.log({
                                    'context': context.getNameAndUUID(),
                                    'dimension': node
                                }, 'GraphWidget.Node.DragMoved'));
                                widget.on('Node.Selected', node => ActivityLogger.log({
                                    'context': context.getNameAndUUID(),
                                    'dimension': node
                                }, 'GraphWidget.Node.Selected'));
                                widget.on('Node.Unselected', node => ActivityLogger.log({
                                    'context': context.getNameAndUUID(),
                                    'dimension': node
                                }, 'GraphWidget.Node.Unselected'));
        ...
        class SurveyWidget {
            ...
            static _makeLikertScaleWidget(labelLow, labelHigh, stepNb, question, stepLabels = [], stepValues = []) {
             ...
             */
            static _makeUserIdWidget(onUserIdChanged) {
            ...
            static _makeInsightWidget(callback) {
        ...
        class ContextQueue {
            ...
            static _makeElem(context = undefined, prev = undefined, next = undefined) {
        ...
        contextQueue.on("ContextDeletedEvent", () => {
            zIndexGenerator = contextQueue.length;
            for (let i of contextQueue) {
